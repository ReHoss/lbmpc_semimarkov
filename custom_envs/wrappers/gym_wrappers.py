...
def exponentially_weighted_penalty(t_max: float,
                                   t: float,
    ...
    """
    norm_action = np.linalg.norm(array_action) ** 2 if squared else np.linalg.norm(array_action)
...
def control_penalty(t_max: float,
                    t: float,
                    array_action: np.array,
                    dict_penalty_config: dict, **kwargs):
    ...
    Args:
        ...
        t (float): The current time.
        array_action (np.array): The action.
        dict_penalty_config (dict): The dictionary containing the control penalty configuration.
            The keys are:
    ...
    if name_penalty == "zero":
        ...
        penalty = exponentially_weighted_penalty(t_max=t_max, t=t, array_action=array_action, squared=True,
                                                 **dict_penalty_config["parameters"])
    elif name_penalty == "l2_norm_exponential_weight":
        penalty = exponentially_weighted_penalty(t_max=t_max, t=t, array_action=array_action, squared=False,
                                                 **dict_penalty_config["parameters"])
    elif name_penalty == "square_l2_norm":
        # Here taking t=t_max returns the standard control penalty.
        penalty = exponentially_weighted_penalty(t_max=t_max, t=t_max, array_action=array_action, squared=True,
                                                 **dict_penalty_config["parameters"])
    elif name_penalty == "l2_norm":
        # Here taking t=t_max returns the standard control penalty.
        penalty = exponentially_weighted_penalty(t_max=t_max, t=t_max, array_action=array_action, squared=False,
                                                 **dict_penalty_config["parameters"])
        ...
        Bu_max = B_hat @ fft.fft(utils.real_flattened_to_complex(np.ones(n_actuators)) * scale)
        norm_radius = np.linalg.norm(Bu_max)
        # array_action is some Bu in fact
        norm_action = np.linalg.norm(array_action)
        penalty = np.array(0.0) if norm_action < norm_radius * epsilon else np.array(1.0)
...
def reward(array_state: np.array, dict_state_reward_config: dict):
    ...
    Args:
        ...
        dict_state_reward_config (dict): The dictionary containing the state reward configuration.
            The keys are:
