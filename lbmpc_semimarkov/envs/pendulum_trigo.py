...
class PendulumEnvTrigo(barl_interface_env.EnvBARL, gym.Env):
    ...
    def __init__(
        ...
        assert (
            dict_scaling_constants["action"] > 0.0
        ), "dict_scaling_constants['action'] should be positive"
        ...
        if self.init_cond_type == "uniform":
            assert (
                self.init_cond_scale == 0.0
            ), "init_cond_scale should be 0.0 for uniform init cond"
        elif self.init_cond_type == "equilibrium":
            assert (
                self.init_cond_scale >= 0.0
            ), "init_cond_scale should be positive for equilibrium init cond"
    ...
    def step(self, u):
        ...
        assert (
            self.current_step_index <= self.horizon
        ...
        # INFO @REMY: I think the change of variable
        # for \theta_here = \theta - \pi is done here.
        # INFO @REMY: Hence vertical orientation is considered;
        # INFO @REMY: we observe the dynamics of y = theta + pi
        # and ydot = xdot in fact; sin(x + pi) = -sin(x)
        newthdot = (
            thdot
            + (
                -3 * g / (2 * pendulum_length) * np.sin(th + np.pi)
                + 3.0 / (m * pendulum_length**2) * force
            )
            * self.dt
        )
        unnorm_newth = th + newthdot * self.dt
        newth = unnorm_newth  # INFO @REMY: warning here HUGE difference !
        # this cause a discontinuity
    ...
    def reset(
        self, obs=None, seed=None, options=None
    ):  # INFO @REMY: modified by Stella to be Gymnasium compliant
        ...
        if obs is None:
            if self.init_cond_type == "uniform":
                ...
                array_noise = self.np_random.normal(
                    loc=0.0, scale=self.init_cond_scale, size=2
                )
