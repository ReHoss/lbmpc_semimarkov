...
class LorenzEnvNew(pde_class.EnvPDE):
    ...
    def __init__(
        ...
        Args:
            ...
            dict_sensing_actuation_config: The dictionary containing
             the configuration of sensing and actuation.
             The keys are:
                    - "actuation_type" (str): The type of actuation. Can be "full"
                     or "partial".
                    - "sensing_type" (str): The type of sensing.
                     Can be "full_observation" or "scaling_noise".
                    - "sensor_std" (float): The standard deviation of the sensor noise.
                     Must be positive.
            ...
            dict_init_condition_config: The dictionary containing the configuration
             ...
             of the initial condition.
                The keys are:
                    - "init_cond_type" (str): The type of initial condition.
                     Can be "random" or "fixed".
                    - "init_cond_scale" (float): The scale of the initial condition.
                     Must be positive.
                    - "index_start_equilibrium" (int): The index of the time step
                     at which the system is at
                        equilibrium. Must be positive.

            dict_reward_config: The dictionary containing the configuration
             of the reward. The keys are:
                - "state_reward" (dict): The dictionary containing the configuration
                 of the state reward. The keys are:
                - "reward_type" (str): The type of reward. Can be "square_l2_norm"
                     or "l2_norm".
                - "control_penalty" (dict): The dictionary containing the configuration
                 of the control penalty.
                    The keys are:
                - "control_penalty_type" (str): The control penalty.
                         Can be "square_l2_norm" or "l2_norm".
                - "parameters" (dict): The dictionary containing the parameters
                 of the control penalty.
                            The keys are:
            ...
            path_rendering: The path to the folder where the rendering will be saved.
             The rendering is done by writing
                the state of the system in a csv file to be read by an external
                 csv renderer.
        ...
        assert set(dict_pde_config.keys()) == {
            "sigma",
            "rho",
            "beta",
            "dt",
        ...
        assert set(dict_init_condition_config.keys()) == {
            "init_cond_type",
            "init_cond_scale",
            "index_start_equilibrium",
        }
        # Assert that the dict_sensing_actuation contains the right keys and only those
        assert set(dict_sensing_actuation_config.keys()) == {
            "actuation_type",
            "sensing_type",
            "sensor_std",
        }

        # If scaling noise is applied,
        # then assert the existence of a positive sensor_std entry
        ...
        # then assert sensor_std is equal to 0
            ...
            assert (
                dict_sensing_actuation_config["sensor_std"] == 0
            ), "For full_obs sensor_std must be equal to 0,"
        # Else raise an error
        else:
            raise ValueError(
                f"Wrong sensing type: {dict_sensing_actuation_config['sensing_type']}"
            )
        ...
        observation_high = 100  # This quantity is a placeholder for infinity,
        # because of the Gym version used in SB3.
        ...
        set_necessary_dict_init_condition_config_keys = {
            "init_cond_type",
            "init_cond_scale",
            "index_start_equilibrium",
        }
        assert (
            set_dict_init_condition_config_keys
            == set_necessary_dict_init_condition_config_keys
        ), (f"Wrong keys in dict_init_condition_config: "
            f"{dict_init_condition_config.keys()}")
        ...
        self.index_start_equilibrium = self.dict_init_condition_config[
            "index_start_equilibrium"
        ]

        self.matrix_equilibria = (
            None  # Matrix containing equilibrium points, set in set_equilibria()
        )
        self.array_equilibria_start = None  # Array containing the barycenter point
        ...
        self.matrix_actuation = (
            None  # Matrix B in the context of Ax + Bu, set in set_actuation()
        )
        self.set_actuation_matrix()
        self.matrix_partial_obs = (
            None  # Matrix C in the context of y = Cx, set in set_sensing()
        )
        ...
        assert (
            dict_reward_config == DICT_FIXED_REWARD_CONFIG
        ), "The reward config does not match the default one."

    # noinspection DuplicatedCode
    def reset(self, **kwargs) -> Union[Tuple, Dict[str, Any], np.ndarray, int]:
        ...
        Args:
            **kwargs: obs (np.ndarray): Initial state of the system.
            If None, then the initial state is generated

        Returns:
            Union[Tuple, Dict[str, Any], np.ndarray, int]: a tuple containing:
        ...
        self.array_scaled_observation = self.scale_observation(
            self.array_unscaled_observation
        )
    ...
    def generate_initial_state(self) -> np.array:
        ...
        Depending on the initial condition type,
         the initial state is generated differently.

        Returns:
            np.array: the initial state of the system
        """
        assert (
            self.init_cond_scale >= 0
        ), f"Value self.init_cond_scale ({self.init_cond_scale}) must be positive."
        ...
        if self.init_cond_type == "random":
            ...
            if self.index_start_equilibrium in [0, 1, 2]:
                ...
                raise ValueError(
                    f"Wrong self.index_start_equilibrium"
                    f" {self.index_start_equilibrium}."
                )
            ...
            raise ValueError(
                f"Wrong self.init_cond_type value ({self.init_cond_type})."
            )
    ...
    def set_equilibria(self):
        ...
        self.matrix_equilibria[1] = (
            -np.sqrt(beta * (rho - 1)),
            -np.sqrt(beta * (rho - 1)),
            rho - 1,
        )
        self.matrix_equilibria[2] = (
            np.sqrt(beta * (rho - 1)),
            np.sqrt(beta * (rho - 1)),
            rho - 1,
        )
    ...
    def set_initial_state(self):
        ...
        if self.index_start_equilibrium in [0, 1, 2]:
            self.array_equilibria_start = self.matrix_equilibria[
                self.index_start_equilibrium
            ]
        else:
            raise ValueError(
                f"Wrong self.index_start_equilibrium {self.index_start_equilibrium}."
            )

    def set_actuation_matrix(self):
        """Set the actuation matrix.

        The actuation matrix is a matrix of size (n_actuators, nx)
         that defines how the actuators act on the system.
        Think of it as a matrix that defines
         the control input of the system in the following way:
        ...
        if self.dict_sensing_actuation_config["actuation_type"] == "full":
        ...
        elif self.dict_sensing_actuation_config["actuation_type"] in [
            "x_0",
            "x_1",
            "x_2",
        ]:
            ...
            index_actuator = int(
                self.dict_sensing_actuation_config["actuation_type"][-1]
            )
            self.matrix_actuation = np.zeros((self.n_actuators, self.nx))
            self.matrix_actuation[index_actuator, index_actuator] = 1
        else:
            raise ValueError(
                f"Wrong self.dict_sensing_actuation_config['actuation_type'] "
            )

    def set_partial_observation_matrix(self):
        """Set the partial observation matrix.

        The partial observation matrix is a matrix of size (nx, nx)
         that defines how the sensors act on the system.
        Think of it as a matrix that defines the observation
        ...
        if self.dict_sensing_actuation_config["sensing_type"] == "scaling_noise":
            ...
            array_noise = self.rng_observation_noise.normal(
                loc=0, scale=sensor_std, size=self.nx
            )
            array_noise = array_noise.astype(self.dtype)
            # Add the array_noise to the diagonal of the matrix
            self.matrix_partial_obs = np.eye(self.nx, dtype=self.dtype) + np.diag(
                array_noise
            )
        else:
    ...
    def step(
        self, array_action: np.ndarray
    ) -> Tuple[Union[Tuple, Dict[str, Any], np.ndarray, int], float, bool, bool, Dict]:
        """Run one timestep of the environment's dynamics. See also, Gymnasium step()
         method (parent class).
        ...
        Returns:
            Tuple[GymObs, float, bool, bool, Dict]: A dictionary containing
             the following items:
                - array_scaled_observation (np.ndarray): the scaled observation of
                 the system
        ...
        self.array_unscaled_action = self.unscale_action(
            array_action=self.array_scaled_action
        )

        if self.current_step % self.control_step_freq == 0:
            # In this case one take a real action
            # INFO @STELLA: spatial mapping
            self.array_control = self.map_action_to_control_state_space(
                array_action=self.array_unscaled_action
            )

        # Update the state
        prev_obs = self.array_state
        self.array_state = self.next_state(
            array_state=self.array_state, array_control=self.array_control
        )
    ...
    def _get_reward(self) -> float:
        ...
        penalty = cost_functions.control_penalty(
            t_max=self.ep_length,
            t=self.current_step,
        ...
        state_reward = cost_functions.reward(
    ...
    def controlled_lorenz_scipy(
        self, time: float, array_state: np.array, array_control: np.array
    ):
    ...
    def next_state(
        self, array_state: np.ndarray, array_control: np.ndarray
    ) -> np.ndarray:
        ...
        ode_result = integrate.solve_ivp(
            fun=self.controlled_lorenz_scipy,
            t_span=[0, self.dt],
            y0=array_state,
            first_step=self.dt,
            method="RK23",
            # Here we pass the control
            atol=10,  # Basically no tolerance in order to perform
            # at most 4 function eval
    ...
    def map_action_to_control_state_space(self, array_action: np.array) -> np.array:
        ...
        In our context, the control u_t is a fonction of the action a_t.
        In other words, u_t = h(a_t).
        This is due to the fact the control must have the same dimension
         as the state while the action
        can have a different dimension. Think as the action as a "high level"
         command and the control
        as a "low level" command. For example, the action can be the intensity
         of the magnetic field
        while the control is the magnetic field itself.

        Here we assume that the control is a linear function of the action.
         In other words, u_t = B a_t.
    ...
    def render(self):
        """Render the environment. See gym.Env.render() for more information.
         But this is a bit different here.

        In our case the rendering is done by the renderer. The renderer is a class
         that is responsible for
        rendering the environment. It is not part of the environment itself.
         The renderer is called by the
        environment at each step.
        The renderer writes the data in a csv file and nothing else. It is intended
