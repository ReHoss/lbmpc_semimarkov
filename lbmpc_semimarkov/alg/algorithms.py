...
class AlgorithmSet:
    ...
    def set_algo_list(self, n_f):
        ...
        self.algo.initialize()  # INFO: @REMY for instance initialise MPC
        self.algo_list = [self.algo.get_copy() for _ in range(n_f)]  # INFO @REMY: this perturbs the random initialisation of the algo; however see note below, they are aware of this
        # INFO @REMY: However they do this to fix the random start state but I don't think it is the optimal way to do it
        """
    ...
    def get_exe_path_and_output_lists(self):
        ...
        output_list = [algo.get_output() for algo in self.algo_list]  # INFO @REMY: Get planned state, action and rewards
    ...
    def get_exe_path_list_crop(self):
        ...
        for algo in self.algo_list:
            exe_path_crop = algo.get_exe_path_crop()  # INFO @REMY: takes the optimal path (self.planned_states and self.planned_actions) and cast it to an execution path ((x_i, a_i)_{i=1}^n, f(x_i, a_i)_{i=1}^n); filter out (crop) the values that are out of the domain
...
class BatchAlgorithm(Algorithm):
    ...
    def set_params(self, params):
        ...
        # self.params.rollout = getattr(params, "rollout", False) # CHANGES @STELLA
    ...
    def run_algorithm_on_f(self, f_batch):
        ...
        """
        self.initialize()  # INFO @REMY: initialise the algo, MPC for instance

        # Step through algorithm
        x_batch = self.take_batch_of_steps(f_batch)
        while len(x_batch) > 0:
            x_batch = self.take_batch_of_steps(f_batch)  # INFO @REMY: the x_batch variable is not very useful here, it is used as a flag to know if the algo is finished or not
    ...
    def take_batch_of_steps(self, f_batch):
        ...
        """
        x_batch = self.get_next_x_batch()  # INFO @REMY: x_batch is a list of size n_samp; it gets a state-action pair (a list of it)

        if len(x_batch) > 0:
            # CHANGES @STELLA: sample sequentially
            # if self.params.rollout:
            #     obs_y_batch = f_batch(x_batch, return_start_obs=True)
            #     obs_y_batch = np.array(obs_y_batch)

            #     obs = obs_y_batch[:, 0]
            #     y_batch = obs_y_batch[:, 1]
            #     obs_dim = obs.shape[1]
            ...
            # else:
            y_batch = f_batch(x_batch)  # INFO @REMY: very important, this will be added to the execution path and used later for marching in time by the MPC artificial planner
            self.exe_path.x.extend(x_batch)
            self.exe_path.y.extend(y_batch)

        return x_batch

    def get_next_x_batch(self):  # INFO @REMY: I think this is misleading and
        """
...
class BatchAlgorithmSet(AlgorithmSet):
    ...
    def run_algorithm_on_f_list(self, f_batch_list, n_f):
        ...
        """
        # Set self.algo_list
        self.set_algo_list(n_f)  # INFO @REMY: x here is considered as the input of f ?

        # Step through algorithms  # INFO @REMY: the algo is the MPC here which gets data inpute
        x_batch_list = [algo.get_next_x_batch() for algo in self.algo_list]  # INFO @REMY: x_batch_list is of size n_f, each entry contains a list containing ndarray of size (n_mpc_action_samples, dim_obs + dim_action)
        while any([len(x_batch) > 0 for x_batch in x_batch_list]):
            y_batch_list = f_batch_list(x_batch_list)  # INFO @REMY: computes f(x) i.e. GP(x, a) = T(x, a); where x contains (state, action); y_batch_list if a 3d nested list of size (n_f, n_mpc_independant_samples, dim_action); this is used for the execution_path
            x_batch_list_new = []
            for algo, x_batch, y_batch in zip(  # INFO @REMY: that part only generates input data
                self.algo_list, x_batch_list, y_batch_list
            ):
                if len(x_batch) > 0:
                    algo.exe_path.x.extend(x_batch)  # INFO @REMY: an execution path is of shape (x_i, f(x_i))_{i=1}^n
        ...
        # Return lists of exe_paths and outputs for each algo in self.algo_list
        return self.get_exe_path_and_output_lists()  # INFO @REMY: returns a tuple; first a list of execution paths, second a list of outputs where output contains planned state, action and rewards
