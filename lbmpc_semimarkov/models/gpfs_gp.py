...
class GpfsGp(SimpleGp):
    ...
    def set_data(self, data, fs_only=False, lmat=None, smat=None):
        """Set self.data."""
        if not fs_only:
            super().set_data(data, lmat=lmat, smat=smat)  # INFO @REMY: notably set Cholesky decomposition of the covariance matrix
        ...
        self.set_model()  # INFO @REMY: warning, here the models are created !

    def set_model(self):
        """Set GPFlowSampling as self.params.model."""
        self.params.model = PathwiseGPR(  # INFO @REMY: PathwiseGPR is a class from gpflow_sampling
...
class MultiGpfsGp(Base):
    ...
    def __init__(self, params=None, data=None, verbose=True):
        ...
        self.gpfsgp_list = []
        self.set_data(data)
        self.set_gpfsgp_list()  # INFO @REMY: populate self.gpfsgp_list with GpfsGp objects having models from GPFlowSampling library
    ...
    def set_data(self, data, fs_only=False, smats=None, lmats=None):
        """Set self.data."""  # INFO @REMY:  data is a Namespace with x and y. x is a nested list like [x_i, a_i], y is a list like [np.array(y_i)]
        if data is None:
    ...
    def initialize_function_sample_list(self, n_samp=1):
        ...
        """   # INFO @REMY: n_samp is the number of GP models
        for gpfsgp in self.gpfsgp_list:
            gpfsgp.initialize_function_sample_list(n_samp)  # INFO @REMY: initialize the GP "paths"
    ...
    def get_post_mu_cov(self, x_list, full_cov=False):
        ...
        mu_list, cov_list = [], []
        for gpfsgp in self.gpfsgp_list:  # INFO @REMY: self.gpfsgp_list is a list of size dim(action_space)
            # Call usual 1d gpfsgp gp_post_wrapper
            mu, cov = gpfsgp.get_post_mu_cov(x_list, full_cov)  # INFO @REMY: returns posterior mean and covariance
            mu_list.append(mu)
            cov_list.append(cov)
    ...
    def gp_post_wrapper(self, x_list, data, full_cov=True):
        ...
        data_list = self.get_data_list(data)  # INFO @REMY: As each output dimension is modeled by independant GP, transform the Namespace data into a list of Namespace of size the state space
        mu_list = []
        cov_list = []
    ...
    def get_data_list(self, data):
        ...
        return data_list  # INFO @REMY: data_list contains a list of Namespace of size the state space
    ...
    def get_gp_params_list(self):
        ...
        """
        gp_params_list = [  # INFO @REMY: self.params.gp_params is a dict with keys 'ls', 'alpha', 'sigma', 'ndimx'
        ...
        return gp_params_list  # INFO @REMY: return a list of dict with keys 'ls', 'alpha', 'sigma', 'ndimx', 'sigma'...
...
class BatchGpfsGp(GpfsGp):
    ...
    def initialize_function_sample_list(self, n_fsamp=1):
        """Initialize a list of n_fsamp function samples."""
        n_bases = self.params.n_bases   # INFO @REMY: n_bases seems to be something like fourier basis
        paths = self.params.model.generate_paths(num_samples=n_fsamp, num_bases=n_bases)  # INFO @REMY: generate_paths is a method of PathwiseGPR class which inhertis from GPMoodel class from gpflow_sampling
        _ = self.params.model.set_paths(paths)
        self.n_fsamp = n_fsamp
    ...
    def call_function_sample_list(self, x_batch_list):
        ...
        """  # INFO @REMY: The docs seems to mean this function computes GP_i(x_j)
        ...
        self.fsl_xvars = np.array(x_batch_list_new)  # INFO @REMY: it overwrites what is done by initialize_fsl_xvars

        # Call fsl on fsl_xvars, return y_list
        y_tf = self.call_fsl_on_xvars(self.params.model, self.fsl_xvars)  # INFO @REMY: y_tf is a tensor of shape (n_fsamp, n_batch, 1), where n_batch is in fact the number of independent samples
        ...
        if type(x_batch_list) is list:
            ...
            for yarr, x_batch in zip(y_tf.numpy(), x_batch_list):
                y_batch = list(yarr.reshape(-1))[: len(x_batch)]  # INFO @REMY: [: len(x_batch)] is here because of the previous cropping
    ...
    def reformat_x_batch_list(self, x_batch_list):
        ...
        dum_val = next(x_batch for x_batch in x_batch_list if len(x_batch) > 0)[0]  # INFO @REMY: this value is a padding value
        max_n_batch = max(len(x_batch) for x_batch in x_batch_list)  # INFO @REMY: extract the size of the longest batch

        # duplicate and reformat each x_batch in x_batch_list, add to x_batch_list_new
        x_batch_list_new = []
        for x_batch in x_batch_list:
            x_batch_dup = [*x_batch]
            x_batch_dup.extend([dum_val] * (max_n_batch - len(x_batch_dup)))  # INFO @REMY: will often be equal to zero
...
class BatchMultiGpfsGp(MultiGpfsGp):
    ...
    def set_gpfsgp_list(self):
        ...
        self.gpfsgp_list = []
        for gpp, dat in zip(gp_params_list, data_list):
            gpfs_gp = BatchGpfsGp(gpp, dat, verb)  # INFO @REMY: creates a unidimensional GP
            self.gpfsgp_list.append(gpfs_gp)
    ...
    def call_function_sample_list(self, x_batch_list):
        ...
        y_batch_list_list = [
            ...
            for gpfsgp in self.gpfsgp_list
        ]  # INFO @REMY: Here the multi GP models are unpacked
        ...
        if type(x_batch_list) is list:
            ...
            ]  # ugly  # INFO @REMY: y_batch_multi_list is of size n_fsamp, each entry is a list of size n_samples for which each entry is of size dim_y
