...
def main():
    ...
    with mlflow.start_run(experiment_id=ml_flow_experiment.experiment_id) as run:
        ...
        # Create the path and directory where to store lbmpc_semimarkov data if needed
        path_barl_data = (
        ...
        start_run(dict_config, path_barl_data=path_barl_data)


def start_run(dict_config: omegaconf.DictConfig, path_barl_data: str):
    # ==============================================
    ...
    # Set algorithm
    algo_class: Type[lbmpc_semimarkov.alg.mpc.MPC] = lbmpc_semimarkov.alg.mpc.MPC
    ...
    test_algo: lbmpc_semimarkov.alg.mpc.MPC = algo_class(params=dict_test_algo_params)
    ...
    namespace_data: argparse.Namespace = get_initial_data(
        ...
        dumper=dumper,
    )  # INFO @REMY: plot_fn is passed because the initial data point will be plotted

    # Make a test set for model evaluation separate from the controller
    ...
    test_data.x = (
        lbmpc_semimarkov.util.domain_util.unif_random_sample_domain(  # list[np.array]
    ...
    # Set model
    gp_model_class: Type[lbmpc_semimarkov.models.gpfs_gp.MultiGpfsGp]
    ...
    for iteration in range(dict_config.num_iters):
        ...
        multi_gp_model: lbmpc_semimarkov.models.gpfs_gp.MultiGpfsGp
        array_xk: np.ndarray
...
def get_initial_data(
    ...
    if dict_config.sample_init_initially:
        ...
        namespace_data.x = lbmpc_semimarkov.util.domain_util.unif_random_sample_domain(
...
def get_model(
    ...
    gp_model_params = {
        "n_dimy": obs_dim,
        "gp_params": gp_params,
        "tf_dtype": lbmpc_semimarkov.util.misc_util.get_tf_dtype(
            dict_config.tf_precision
        ),
    }
    gp_model_class = lbmpc_semimarkov.models.gpfs_gp.BatchMultiGpfsGp
...
def get_acq_fn(
    ...
    if dict_config.alg.uncertainty_sampling:
        ...
        if not dict_config.alg.rollout_sampling:
            # standard lbmpc_semimarkov
            acqfn_class = MultiBaxAcqFunction
        else:
            # new rollout lbmpc_semimarkov
            acqfn_params["gp_model_params"] = gp_model_params
            if dict_config.alg.joint_eig:
...
def get_acq_opt(
    ...
    if dict_config.alg.rollout_sampling:
        ...
        acqopt_class = (
            lbmpc_semimarkov.acq.acqoptimize.PolicyAcqOptimizer
        ...
        if getattr(dict_config.alg, "max_interdecision_epochs", None) is not None:
            acqopt_class = lbmpc_semimarkov.acq.acqoptimize.AcqOptimizer
...
def fit_hypers(
    ...
    if gp_model_params is None:
        ...
        # Plot hyper fitting data  # TODO: improve this from initial repo
        ...
        for idx in tqdm.trange(len(fit_data.y[0])):  # Coordinate-wise (output) fitting
            ...
            gp_params = lbmpc_semimarkov.models.gpflow_gp.get_gpflow_hypers_from_data(
                data_fit,
    ...
    model = lbmpc_semimarkov.models.gpfs_gp.BatchMultiGpfsGp(gp_model_params, fit_data)
...
def execute_gt_mpc(
    algo_class: Type[lbmpc_semimarkov.alg.mpc.MPC | Any],
    ...
    true_algo: lbmpc_semimarkov.alg.mpc.MPC = algo_class(dict_algo_params)
    ...
    for id_eval_trial in tqdm_progress_bar:
        ...
        plt_figure_groundtruth, plt_axes_groundtruth = (
            visualisation.plot_ground_truth.plot_ground_truth(
                env=env,
        ...
        list_returns.append(
            lbmpc_semimarkov.util.control_util.compute_return(tuple_output[2], 1)
        )
        stats = {
...
def get_next_point(
    iteration: int,
    dict_config: omegaconf.DictConfig,
    algo: lbmpc_semimarkov.alg.mpc.MPC,
    list_tuple_domain: list[tuple[float, float]],
    array_xk: np.ndarray,
    action_space: gymnasium.spaces.Box,
    gp_model_class: Type[lbmpc_semimarkov.models.gpfs_gp.MultiGpfsGp],
    gp_model_params: dict,
    acqfn_class: Type[lbmpc_semimarkov.acq.acquisition.MultiBaxAcqFunction],
    acqfn_params: dict,
    acqopt_class: Type[lbmpc_semimarkov.acq.acqoptimize.AcqOptimizer],
    acqopt_params: dict,
    ...
    lbmpc_semimarkov.models.gpfs_gp.MultiGpfsGp | None,
    np.array,
    float,
]:
    multi_gp_model: lbmpc_semimarkov.models.gpfs_gp.MultiGpfsGp | None = None
    ...
    if (len(namespace_data.x) == 0) and (not dict_config.alg.rollout_sampling):
        ...
        multi_gp_model: lbmpc_semimarkov.models.gpfs_gp.MultiGpfsGp = gp_model_class(
    ...
    if dict_config.alg.use_acquisition:
        ...
        # INFO @REMY: something like lbmpc_semimarkov.models.gpfs_gp.BatchMultiGpfsGp;
        # data is the dataset D = {(x_i, y_i)}_{i=1}^n we carefully choose
        ...
        acqopt: lbmpc_semimarkov.acq.acqoptimize.AcqOptimizer = acqopt_class(
        ...
        if dict_config.alg.rollout_sampling:
            ...
            list_array_candidate_state_actions += (
                lbmpc_semimarkov.util.domain_util.unif_random_sample_domain(
                    list_tuple_domain, n=n_rand
                )
            )

            list_posterior_returns: list[float] = [
                lbmpc_semimarkov.util.control_util.compute_return(output[2], 1)
        ...
        policy_function: Callable = functools.partial(
            algo.execute_mpc,
            f=lbmpc_semimarkov.util.misc_util.make_postmean_fn(
...
def evaluate_mpc(  # TODO: Understand this function
    ...
    if model is None:
        return
    with lbmpc_semimarkov.util.timing.Timer("Evaluate the current MPC policy") as timer:
        ...
        postmean_fn = lbmpc_semimarkov.util.misc_util.make_postmean_fn(
        ...
        if dict_config.eval_bayes_policy:
            ...
            policy = lbmpc_semimarkov.policies.BayesMPCPolicy(params=policy_params)

        else:
            policy = functools.partial(
                # INFO @REMY: partial is a function that takes a function
        ...
        for j in pbar:
            ...
            real_obs, real_actions, real_rewards = (
                lbmpc_semimarkov.util.control_util.evaluate_policy(
            ...
            real_return = lbmpc_semimarkov.util.control_util.compute_return(
                real_rewards, 1
            )
            ...
            mses.append(
                lbmpc_semimarkov.util.misc_util.mse(
                    real_path_mpc.y, real_path_mpc.y_hat
                )
            )
            stats = {
        ...
        if test_data is not None:
            ...
            random_mse = lbmpc_semimarkov.util.misc_util.mse(test_data.y, test_y_hat)
            random_likelihood = lbmpc_semimarkov.util.misc_util.model_likelihood(
                model, test_data.x, test_data.y
            )
            gt_mpc_y_hat = postmean_fn(namespace_test_mpc_data.x)
            gt_mpc_mse = lbmpc_semimarkov.util.misc_util.mse(
                namespace_test_mpc_data.y, gt_mpc_y_hat
            )  # INFO @REMY: test_mpc_data comes from the GT trajectories
            # at the begining
            gt_mpc_likelihood = lbmpc_semimarkov.util.misc_util.model_likelihood(
...
def sample_forward_points(
    acqopt: lbmpc_semimarkov.acq.acqoptimize.AcqOptimizer,
    array_xk: np.ndarray,
    n_rand_acqopt: int,
    ...
    ndarray_trajectory[:, n_dt, :, :] = (
        matrix_initial_points  # will be broadcast to the whole tensor
    )
    ...
    if acqopt.acqfunction.algorithm.params.project_to_domain:
        ...
        matrix_mean_trajectory = np.array(
            [
                lbmpc_semimarkov.util.domain_util.project_to_domain(
