...
class AcqFunction(Base):
    ...
    def set_model(self, model):
        ...
        if not model:
            ...
            self.model = copy.deepcopy(model)  # INFO @REMY: the model can be a BatchMultiGpfsGp
...
class AlgoAcqFunction(AcqFunction):
    ...
    def __init__(self, params=None, model=None, algorithm=None, verbose=True):
        ...
        super().__init__(params, verbose)
        self.set_model(model)
        self.set_algorithm(algorithm)  # INFO @REMY: e.g. MPC object
    ...
    def initialize(self):
        ...
        the model.
        """
        exe_path_list, output_list, full_list = self.get_exe_path_and_output_samples()  # INFO @REMY: full_list is the list of execution paths (Namespace) (x_i, a_i)_{1<=i<=n}, f(x_i, a_i)_{1<=i<=n} for each GP_i; exe_path_list is the conversation of \tau^*_i converted as execution paths, hence a list of execution paths (x_i, a_i)_{1<=i<=n}, f(x_i, a_i)_{1<=i<=n} for each GP_i cropped to the domain boundaries; output_list is the list of \tau^*_i for each GP_i
    ...
    def get_exe_path_and_output_samples_loop(self):
        ...
        exe_path_list = []
        output_list = []
        with Timer(f"Sample {self.params.n_path} execution paths") as timer:
        ...
        # CHANGES @STELLA: save execution paths timing
        if self.params.dumper: self.params.dumper.add("Time/Sample n execution paths", timer.time_elapsed)
    ...
    def get_exe_path_and_output_samples(self):
        ...
        with Timer(f"Sample {self.params.n_path} execution paths") as timer:
            ...
            self.model.initialize_function_sample_list(self.params.n_path)  # INFO @REMY: self.params.n_path is the number of differents GP_i to sample from

            if getattr(self.algorithm.params, "is_batch", False):
                algoset = BatchAlgorithmSet(self.algorithm)  # INFO @REMY: self.algorithm can be a MPC object
            else:
                algoset = AlgorithmSet(self.algorithm)

            # Run algorithm on function sample list
            f_list = self.model.call_function_sample_list  # INFO @REMY: f_list is a method from BatchMultiGpfsGp, it computes GP_i(x_j) "Multi" stands for "GP_i" and batch for "GP_i(x_j)"
            ...
            exe_path_list = algoset.get_exe_path_list_crop()  # INFO @REMY: returns a list for each of the GP_i, each list contains the execution path (x_i, a_i)_{1<=i<=n}, f(x_i, a_i)_{1<=i<=n}; the crop means filtering out what is not in the domain boundaries
...
class BaxAcqFunction(AlgoAcqFunction):
    ...
    def get_acq_list_batch(self, x_list):
        ...
        # CHANGES @STELLA: save EIG calculation timing
        if self.params.dumper: self.params.dumper.add("Time/Acquisition function", timer.time_elapsed)
...
# INFO @STELLA: standard BARL acquisition function
class MultiBaxAcqFunction(AlgoAcqFunction):
    ...
    def acq_exe_normal(self, post_stds, samp_stds_list):
        ...
        # INFO @STELLA: EIG calculation
        :param post_stds: posterior stds
        :param samp_stds_list: list of sampled trajectory j stds 
        """
        # logging.info("EIG calculation")
        # Compute entropies for posterior predictive
        # H[s' | s,a,D]
        h_post = self.entropy_given_normal_std_list(post_stds)  # INFO @REMY: h_post is an np.array of size minibatch

        # Compute entropies for posterior predictive given execution path samples
        h_samp_list = []  # INFO @REMY: this is a list of
    ...
    def get_acq_list_batch(self, x_list):
        ...
        with Timer(
            ...
            # a list of stds
            mus, stds = self.model.get_post_mu_cov(x_list, full_cov=False)  # INFO @REMY: the list is of size dim(action)
            ...
            mus_list = []
            stds_list = []
            for exe_path in self.exe_path_list:  # INFO @REMY: self.exe_path_list is a list of execution paths (x_i, a_i)_{1<=i<=n}, f(x_i, a_i)_{1<=i<=n} for each GP_i cropped to the domain boundaries, hence it is of size n_gp
                comb_data = Namespace()
                comb_data.x = self.model.data.x + exe_path.x  # INFO @REMY: union of the data points of the GP_i and the execution path (x_i, a_i)_{1<=i<=n}
                comb_data.y = self.model.data.y + exe_path.y
                # INFO @REMY: Optimal trajectory is in self.exe_path_list !
                # NOTE: self.model is multimodel so the following returns a list of mus
                # and a list of stds
                samp_mus, samp_stds = self.model.gp_post_wrapper(  # INFO @REMY: Note that contrary to the computations above, here the optimal trajectory is added to the data points
                    x_list, comb_data, full_cov=False  # INFO @REMY: it is enough to get the mean and std for each prediction of the transition kernel (GP), as the std will characterise the entropy of each of the cond. distributions
                )
                mus_list.append(samp_mus)
                stds_list.append(samp_stds)

            # Compute acq_list, the acqfunction value for each x in x_list
            acq_list = self.acq_exe_normal(stds, stds_list)  # INFO @REMY: EIG(x) calculation; acq_list is of size n_minibatch
        # CHANGES @STELLA: save EIG calculation timing
        if self.params.dumper: self.params.dumper.add("Time/Acquisition function", timer.time_elapsed)
...
class MCAcqFunction(AcqFunction):
    ...
    def __init__(self, wrapped_acq_function, params):
        ...
        self.num_samples_mc = self.params.num_samples_mc # sample pairs
        self.acq_function_copies = [ # acquisition functions for every (s,a) pair
    ...
    def initialize(self):
        ...
        for fn in self.acq_function_copies:
            ...
            self.exe_path_full_list += fn.exe_path_full_list # get output_samples via AlgoAcqFunction
    ...
    def __call__(self, x_list):
        ...
        lists = []
        for fn in self.acq_function_copies:
            lists.append(fn(x_list)) # call acq function -> returns acq_fn values
