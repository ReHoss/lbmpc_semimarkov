...
class AcqOptimizer(Base):
    ...
    def set_params(self, params):
        ...
        # CHANGES @STELLA: percentage of points to return
        self.params.keep_samples_perc = getattr(params, "keep_samples_perc", .0)
    ...
    def optimize(self, x_batch):
        ...
        """
        self.params.x_batch = x_batch  # INFO @REMY: x_batch is a list of (s,a) pairs previously sampled
...
class PolicyAcqOptimizer(AcqOptimizer):
    ...
    def get_initial_mean(self, action_sequence):
        if action_sequence is None:
            ...
            return new_action_sequence[: self.params.planning_horizon, ...]  # INFO @REMY: I think this indexing is not necessary as it is equal to the whole array
    ...
    def optimize(self, x_batch=None):
        ...
        with Timer(
            ...
            for i in range(self.params.num_s0_samps):
                s0, _ = self.params.s0_sampler()  # INFO @REMY Should not be reached if x_batch is not None
    ...
    def _optimize(self, x_batch):
        ...
        current_obs = x_batch[0][: self.params.obs_dim]  # INFO @REMY: it looks like x_batch is useless...
        horizon = self.params.planning_horizon
        beta = self.params.beta
        mean = self.get_initial_mean(self.params.action_sequence)  # INFO @REMY: mean is a ndarray of shape (planning horizon h, action_dim)
        if self.params.actions_until_plan > 0:  # INFO @REMY: this march in time the system without planning
            self.params.action_sequence = mean  # INFO @REMY: the mean replace the action sequence attribute, the mean is and array like [self.action_sequence[1:], np.zeros(1)] (a shift)
            query = np.concatenate([current_obs, mean[0, :]])
            self.params.actions_until_plan -= 1
            return query, 0.0  # INFO @REMY: set a zero return
