...
class PendulumEnvSemiMarkovNew(gym.Env):
...
def pendulum_semimarkov_reward_new(x, next_obs, current_step):
    del current_step
    # Assert x has 3 dimensions
    assert x.shape[-1] == 3, f"x.shape={x.shape} should have 3 dimensions"
    th = next_obs[..., 0]
    thdot = next_obs[..., 1]
