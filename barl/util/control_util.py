...
def get_f_mpc(env, use_info_delta=False):
    ...
    # CHANGES @STELLA: added option to return start observation
    def f(x):
        x = np.array(x)
        obs = x[:obs_dim]
        action = x[obs_dim:]
...
def iCEM_generate_samples(
    ...
    # CHANGES @REMY: Start - add random seed
    local_seed = np.random.randint(1e12)  # This seed will be affected by the global seed
    random_state = np.random.default_rng(local_seed)
    # CHANGES @REMY: End
    samples = (  # INFO @REMY: ndarray of shape (nsamps, MPC horizon, action_dim)
        colorednoise.powerlaw_psd_gaussian(
            beta, size=(nsamps, action_dim, horizon), random_state=random_state
        ).transpose([0, 2, 1])
        * np.sqrt(var)
        + mean
    )
...
class EnvDynamicsUnroller(DynamicsUnroller):
    ...
    def unroll(self, start_obs, action_samples):
        observations = [self._env.reset(start_obs)[0]]
        rewards = []
...
class ResettableEnv(gym.Env):
    ...
    def reset(self, obs=None, **kwargs):
        reset_obs, info = self._wrapped_env.reset(**kwargs)
...
def rollout_cem_continuous_cartpole(env, unroller):
    ...
    for _ in trange(env_horizon):
        ...
        action = actions[0]
        start_obs, rew, terminated, truncated, info = env.step(action)
        done = terminated or truncated
        rewards.append(rew)
        if done:
            break
...
def evaluate_policy(policy, env, start_obs=None, mpc_pass=False, autobatch=False, dumper=None):
    obs, info = env.reset(start_obs)  # INFO @REMY: obs is ndarray of shape (obs_dim,)
    observations = [obs]  # INFO @REMY: those are lists to store the trajectory
    actions = []
    rewards = []
    done = False
    samples_to_pass = []
    for _ in range(env.horizon):
        if autobatch:
            obs = obs[None, :]
        if not mpc_pass:  # INFO @REMY: mpc_pass may mean that the policy is a MPC policy
            with Timer("Get action from policy", verbose=False) as timer:
                action = policy(obs)
        else:
            with Timer("Get action from policy", verbose=False) as timer:
                action, samples_to_pass = policy(
                    obs, samples_to_pass=samples_to_pass, return_samps=True  # INFO @REMY: samples_to_pass is list; it can be empty
                )
        if dumper: dumper.add("Action time", timer.time_elapsed, verbose=False)
        if autobatch:
            action = action[0, :]
        obs, rew, terminated, truncated, info = env.step(action)  # INFO @REMY: Changed by Stella
        done = terminated or truncated
        observations.append(obs)
        actions.append(action)
        rewards.append(float(rew))
        if done:
            obs, info = env.reset(start_obs)  # INFO @REMY: Added by Stella ? Not necessary ? I don't think so
            break
