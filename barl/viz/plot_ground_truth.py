...
SET_ENVIRONMENTS_NAME = ["bacpendulum-trigo-v0", "ks-v0"]
...
def plot_ground_truth_pendulum_trigo(
    ...
    matrix_state_action: np.array = np.array(namespace_true_path.x)
    matrix_state: np.array = matrix_state_action[:, :dim_state]
    matrix_action: np.array = matrix_state_action[:, dim_state:]
    matrix_action_unscaled: np.array = matrix_action * action_max_value

    array_time_axis: np.array = np.linspace(0, env.dt * env.horizon, length_time_series)
