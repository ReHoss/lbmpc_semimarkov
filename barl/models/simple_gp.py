...
class SimpleGp(Base):
    ...
    def set_kernel(self, params):
        ...
        self.params.kernel_str = getattr(params, "kernel_str", "rbf")  # INFO @REMY: default kernel is rbf; this will be passed to the construtor
    ...
    def set_data(self, data, lmat=None, smat=None):
        """Set self.data."""  # INFO @REMY: data is a Namespace with x of dim dim(state)+ dim(action) and y of dim dim(state)
        if data is None:
            ...
            data = dict_to_namespace(data)  # INFO @REMY: simply will be casted to Namespace
    ...
    # INFO @STELLA: this is where stds are calculated (k2 with full_cov=False)
    def get_post_mu_cov(self, x_list, full_cov=True):
        ...
        cov : ndarray
            ...
            If full_cov is True, return the covariance matrix as a numpy ndarray
            (len(x_list) x len(x_list)).
        """  # INFO @REMY: self.data is a Namespace with x of dim dim(state) + dim(action) and y of dim dim(state)
        k21 = self.params.kernel(x_list, self.data.x, self.params.ls, self.params.alpha)  # INFO @REMY: computes K(x, x^*) (notations from Rasmussen et al. (2006))
        mu2 = k21.dot(self.smat)  # INFO @REMY: self.smat is a scalar because it is the first iter in this case, but self.smat should be of dimension (n, n) where n is the number of data points in self.data.x
        # INFO @REMY: 21; 22 is the position in the block matrix of Rasmussen et al. (2006)
    ...
    def gp_post_wrapper(self, x_list, data, full_cov=True):
        ...
        if len(data.x) == 0:  # INFO @REMY: data is a Namespace with x of dim dim(state) + dim(action) and y of dim dim(state)
            return self.get_prior_mu_cov(x_list, full_cov)
        # If data is not empty:
        mu, cov = gp_post(
            data.x,  # INFO @REMY: list of data points of size (n_horizon + 1, dim(state) + dim(action))
            data.y,
            x_list,  # INFO @REMY: list of data points of size (n_minibatch, dim(state) + dim(action))
            self.params.ls,
            self.params.alpha,
            self.params.sigma,
            self.params.kernel,
